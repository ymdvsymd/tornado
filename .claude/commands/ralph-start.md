...
> **Note:** Verification Scope 指示と Plan Context は自動注入される。Claude が直接出力する必要はない。
...
> **Note:** Phase 3 の node スクリプトで以下が自動注入される:
>
> - `goal` 側: Verification Scope 指示（Wave 単位の検証スコープ制約）+ Plan Context → Verifier が当該 Wave のタスクのみ検証
...
#### 3-2. milestones.json（Wave スコープ指示 + Plan Context 注入）

bash/node スクリプトで Wave スコープ指示と `plan-en.md` の内容を各フィールドに注入し、`milestones.json` を生成する:

- `goal` 側: Wave スコープ指示 + Plan Context（Verifier が Wave 単位で正しく検証するため）
- `description` 側: Plan Context のみ（Builder 用、変更なし）

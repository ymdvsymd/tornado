...
argument-hint: plan_file:計画ファイルの絶対パス
---
...
```
/ralph-whirlwind </absolute/path/to/plan-file.md> [--planner=<kind>] [--builder=<kind>] [--verifier=<kind>]
```
...
1. **入力検証** — 引数解析、絶対パス検証、ファイル確認、リポジトリルート取得
...
1. `$ARGUMENTS` を解析する:
   ...
   - 引数が空、または `PLAN_FILE` トークンがない場合はエラーメッセージを出して終了

2. `PLAN_FILE` が絶対パス（`/` で始まる）であることを検証する。相対パスの場合はエラーメッセージを出して終了

3. `PLAN_FILE` のファイル存在を確認する。存在しない場合はエラーメッセージを出して終了

4. リポジトリルートを取得し `REPO_ROOT` に格納する:
...
1. 実行コマンドを**1つだけ**組み立てる。変換処理や中間 JSON 生成は行わない。
   ...
   ```bash
   bin/whirlwind.js --plan=<PLAN_FILE> [--planner=<PLANNER_KIND>] [--builder=<BUILDER_KIND>] [--verifier=<VERIFIER_KIND>] --run-log
   ```

2. 上記コマンドをバックグラウンドで起動する。
   - `--plan` には Phase 1 で検証済みの `PLAN_FILE`（絶対パス）をそのまま渡す
   - `--planner` / `--builder` / `--verifier` は、引数で指定された場合のみコマンドに含める。省略された kind のフラグは付与しない
   - `--run-log` フラグでランログを有効化する（ログパスは CLI が自動決定）
...
- 計画ファイルのパスが絶対パスでない場合 → エラーメッセージを出して終了

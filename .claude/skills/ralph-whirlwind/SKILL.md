...
- `--dev=<kind>`: Builder agent kind。デフォルトは `claude-code`
- `--verifier=<kind>`: Verifier agent kind。デフォルトは `codex`
...
1. `$ARGUMENTS` を解析する:
   ...
   - `--dev=<value>` があれば `BUILDER_KIND` に格納（デフォルト: `claude-code`）
   - `--verifier=<value>` があれば `VERIFIER_KIND` に格納（デフォルト: `codex`）
